namespace Dreamteck.Forever
{
    ...
    public partial class LevelSegment : MonoBehaviour
    {
        ...
        public void Extrude()
        {
            ...
            {
                path.samples[0].CopyFrom(_previous.path.samples[_previous.path.samples.Length - 1]);
        ...
        private void GetExtrudeResult(Vector3 boundsPercent)
        {
            ...
            switch (axis)
            {
                case Axis.X: Evaluate(boundsPercent.x, extrudeResult); break;
                case Axis.Y: Evaluate(boundsPercent.y, extrudeResult); break;
                case Axis.Z: Evaluate(boundsPercent.z, extrudeResult); break;
            }
        ...
        public void GetExtrudeResult(Vector3 boundsPercent, SplineSample result, ref Quaternion rotation)
        {
            ...
            switch (axis)
            {
                case Axis.X: Evaluate(boundsPercent.x, result); break;
                case Axis.Y: Evaluate(boundsPercent.y, result); break;
                case Axis.Z: Evaluate(boundsPercent.z, result); break;
            }
